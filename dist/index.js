...
class NativeBarcodeFindViewProxy extends scanditDatacaptureFrameworksCore.BaseNativeProxy {
    ...
    setPositionAndSize(top, left, width, height, shouldBeUnderWebView) {
        return new Promise((resolve, reject) => {
            NativeBarcodeFindViewProxy.cordovaExec(resolve, reject, CordovaFunction.SetFindViewPositionAndSize, [{ top: top, left: left, width: width, height: height, shouldBeUnderWebView: shouldBeUnderWebView }]);
        });
    }
    subscribeBarcodeFindViewListener() {
        return new Promise((_resolve, reject) => {
            NativeBarcodeFindViewProxy.cordovaExec(this.notifyListeners.bind(this), (error) => {
                reject(error);
            }, CordovaFunction.RegisterBarcodeFindViewListener, null);
        });
    }
...
(function (CordovaFunction) {
    ...
    CordovaFunction["SetFindViewPositionAndSize"] = "setFindViewPositionAndSize";
...
class BarcodeFindView {
    ...
    hide() {
        if (this.htmlElement) {
            throw new Error("Views should only be manually hidden if they're manually sized using setFrame. Call detachFromElement() first.");
        }
        return this._hide();
    }
